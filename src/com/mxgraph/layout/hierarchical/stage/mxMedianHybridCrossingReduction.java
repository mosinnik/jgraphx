...
public class mxMedianHybridCrossingReduction implements
    ...
    private int calculateCrossings(mxGraphHierarchyModel model) {
        ...
        for (int i = 1; i < numRanks; i++) {
            ...
            int oldCalcRank = 0;
//            int oldCalcRank = calculateRankCrossing(i, model);
            long t2 = System.currentTimeMillis();
            int newCalcRank = calculateRankCrossing2(i, model);
            long t3 = System.currentTimeMillis();
//            if (oldCalcRank != newCalcRank)
//                throw new RuntimeException(i + " -> " + oldCalcRank + " - " + newCalcRank + " -> " + (oldCalcRank - newCalcRank));

            oldTimeMs = oldTimeMs + (t2 - t1);
            newTimeMs = newTimeMs + (t3 - t2);
//            System.out.println(i + " -> " + oldCalcRank + " - " + newCalcRank + " -> " + (oldCalcRank - newCalcRank) + "\t\t\t" + oldTimeMs + " - " + newTimeMs);
            totalCrossings += newCalcRank;
        }
     ...
     * @return the number of edges crossings with the rank beneath
     */
    protected int calculateRankCrossing2(int i, mxGraphHierarchyModel model) {
...
//        System.out.println("   previousRank.size()=" + previousRank.size());
...
//            System.out.println("   " + index + " -> " + firstIndex + " = " + treeIndex);
